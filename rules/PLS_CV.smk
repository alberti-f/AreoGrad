...
rule pls_cv:
    ...
    output:
        f"{OUTDIR}/{OUTSUBDIR}/PLS/CV_results_G1-G{{n_gradients}}.rs{config['random_state']}.h5",
    params:
...
rule pls_cv_perm:
    input:
        ...
        dispersion_path = f"{OUTDIR}/{OUTSUBDIR}/All.rFC_Dispersion.{PARCELLATION}_{SCALE}.npy"
    output:
        f"{OUTDIR}/{OUTSUBDIR}/PLS/permutations/CV_results_G1-G{{n_gradients}}_perm{{i_perm}}.rs{config['random_state']}.h5",
    params:
        random_state = config["random_state"],
        n_splits = config["n_splits"]
    resources:
...
rule pls_perm_test:
    input:
        true_path = f"{OUTDIR}/{OUTSUBDIR}/PLS/CV_results_G1-G{{n_gradients}}.rs{config['random_state']}.h5",
